...
# doing the same thing but for family:
ps_family_glom = tax_glom(phyloseq, 'Family')

# need to make a whole new function for it later
dose_levels_2 = unique(sample_data(ps_family_glom)$dose_categorical)

all_DA_results = lapply(dose_levels_2, ancombc2_by_dose) %>%
  bind_rows()
